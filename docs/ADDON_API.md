...
El framework cliente vigente es `GlobalStorageSiK.SiK_UI`: `GS_SiK_UI_Core.lua` es su sustrato de render y primitivas, mientras Metrics, Viewport, Window, Modal, Block, Controls, List, Table, State, Palette y TerminalScroll componen la API completa. Sustituye por completo los nombres retirados Chrome/`createNeat*`; no existe un shim. Su superficie, límites y ciclo de vida están en [SIK_UI_API.md](SIK_UI_API.md). Los addons oficiales deben usar únicamente los componentes allí documentados y no acceder a campos `_sik*`/`_gs*` ni a pools o hosts internos.

Desde Core `1.3.86-dev32`, el scroll normal y la lista virtual son también propios: `GlobalStorageSiK.TerminalScroll` concentra desplazamiento/barra/recorte. Los addons empiezan por `List`, `Block` o `Table.createBlock`; `SiK_UI.VirtualList.create(...)` se conserva solo como alias de compatibilidad y no se recomienda a consumidores nuevos.

Desde `1.3.86-dev33`, `GlobalStorageSiK.SiK_UI.Table` recibe descriptores de columnas parametrizables (`key`, título, ancho fijo/fraccional/flexible, mínimos, alineación, padding y valores a medir) y comparte el layout resultante entre cabecera, filas y clics. Las métricas se resuelven con `Table.metrics(...)` y los contratos de Metrics, con sobrescrituras por instancia. `Table.createBlock` compone ya una tabla completa; su aceptación visual sigue requiriendo paridad runtime contra el HTML validado. `SiK_UI.Modal/Window` centraliza confirmaciones, altura por contenido, Escape y geometría de editores; `SiK_UI.Palette` gobierna únicamente la apariencia local.
