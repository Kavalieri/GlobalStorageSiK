...
| `DebugCatRuleMigration` | `>> Migración de reglas legacy` / `>> Legacy rule migration` | Saneado de basura y migración externa separados: captura, dos pasadas, equivalencia de catálogo, postvalidación y marcadores independientes; hasta tres muestras acotadas de `rules` o `categories`. |
...
Para una categoría externa conocida, la misma opción añade `externalCategoryMigration phase=capture`. El informe compara antes de escribir el conjunto completo de tipos externos con la ruta nativa: `missingNative=0`, `unexpectedNative=0`, `missingCatalog=0` y `equivalent=true` son obligatorios. Solo entonces aparecen `pass1`, `pass2` con `changedOwners=0` y `postvalidate matches=0`, seguidos de `externalCategoryMigrationMarker ... version=1 status=written`. Una diferencia o catálogo no disponible deja `status=withheld` sin mutar nada. El valor previo y su procedencia quedan recuperables en `externalCategoryMigrationHistory`; una clave externa no reconocida se conserva y se informa mediante `unknownPreserved`, nunca se manda a `legacyJunkRules`.
