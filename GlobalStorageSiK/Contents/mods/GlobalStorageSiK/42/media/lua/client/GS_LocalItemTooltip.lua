...
local function resolve(row, terminal)
    ...
    if not gs.Zones or not gs.Zones.getRegistry or not gs.Network
        or not gs.Network.findWorldObject or not gs.Utils or not gs.Utils.getObjectContainer
        or not gs.Permissions or not gs.Permissions.canAccessZone then return nil end
    ...
    if not node or not zone or zone.networkId ~= state.networkId or zone.enabled == false
        or node.enabled == false or node.offline == true or node.membership == "excluded" then return nil end
    local player = getSpecificPlayer and getSpecificPlayer(terminal.playerNum or 0)
    if not player or not gs.Permissions.canAccessZone(player, state.networkId, node.zoneId) then return nil end
