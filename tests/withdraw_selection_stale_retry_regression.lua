...
local function exerciseVisualState(label, visualFields, initialRevision)
	...
	assert(GlobalStorageSiK.WithdrawClient.onTerminalState({
		...
		snapshotCertified = false, items = { freshRow },
	}) == false, label .. ": uncertified newer snapshot was incorrectly consumed")
	assert(GlobalStorageSiK.WithdrawClient.onTerminalState({
		networkId = "network-a", inventoryRevision = freshRevision,
...
-- A read-only recapture is allowed 30s, unlike the 10s transfer ACK deadline.
-- It must not resend while waiting and must expire only at its own deadline.
local refreshWaitRow = {
	rowKey = "group:Base.VHS:214", fullType = "Base.VHS", count = 2,
	selectionMode = "exact_group", selectionRevision = 45,
}
assert(GlobalStorageSiK.WithdrawClient.sendWithdraw(refreshWaitRow, 0, "container:trunk", "woodcraft",
	{ networkId = "network-a" }))
	tick()
local refreshWaitRequest = lastCommand("withdrawItem")
assert(staleResult(refreshWaitRequest) == true, "30s refresh fixture did not enter stale wait")
local refreshWaitCommands = #commands
now = now + 10000; tick()
assert(GlobalStorageSiK.WithdrawClient.isPending() and #commands == refreshWaitCommands,
	"recapture wait incorrectly expired at the 10s transfer deadline")
now = now + 19999; tick()
assert(GlobalStorageSiK.WithdrawClient.isPending() and #commands == refreshWaitCommands,
	"recapture wait resent or expired before 30s")
now = now + 1; tick()
assert(not GlobalStorageSiK.WithdrawClient.isPending() and tick == nil and #commands == refreshWaitCommands,
	"recapture wait did not cleanly expire at 30s")

-- Equal revisions are accepted only when explicitly certified. A node-scoped
-- gesture additionally requests one authoritative node capture before retry.
local scopedRow = {
	rowKey = "group:Base.VHS:214", fullType = "Base.VHS", count = 2,
	selectionMode = "exact_group", selectionRevision = 50,
	sourceNodeId = "node-a",
}
assert(GlobalStorageSiK.WithdrawClient.sendWithdraw(scopedRow, 0, "container:trunk", "woodcraft",
	{ networkId = "network-a" }))
	tick()
	local scopedRequest = lastCommand("withdrawItem")
	assert(staleResult(scopedRequest) == true, "scoped gesture did not enter stale wait")
	local beforeNodeRefresh = #commands
assert(GlobalStorageSiK.WithdrawClient.onTerminalState({
	networkId = "network-a", inventoryRevision = 50, snapshotCertified = true,
	sourceNodeId = nil, items = { scopedRow },
}) == false, "node-scoped retry accepted an aggregate snapshot")
assert(#commands == beforeNodeRefresh + 1 and lastCommand("getNodeContents").nodeId == "node-a",
	"node-scoped retry did not request exactly one node refresh")
assert(GlobalStorageSiK.WithdrawClient.onTerminalState({
	networkId = "network-a", inventoryRevision = 50, snapshotCertified = true,
	sourceNodeId = "node-a", items = { scopedRow },
}) == true, "equal certified revision was not accepted")
tick()
assert(lastCommand("withdrawItem").selectionRevision == 50,
	"equal certified revision retry changed the selection revision")
GlobalStorageSiK.WithdrawClient.cancelAll("scope_test_done")
