...
assert(model({ items = {}, scanStatus = { state = "RUNNING" } }).emptyText == "",
	"RUNNING provisional snapshot falsely reported an empty inventory")
assert(model({ items = {}, scanStatus = { state = "STALE_RETRY" } }).emptyText == "",
	"invalidated capture falsely reported an empty inventory")
assert(model({ items = {}, reconcilePending = true }).emptyText == "",
	"pending reconciliation falsely reported an empty inventory")
assert(model({ items = { { rowKey = "prior-row" } }, scanStatus = { state = "RUNNING" } }).emptyText
	== "IGUI_GS_NoFilterMatches", "recapture hid the prior nonempty inventory's filter state")
