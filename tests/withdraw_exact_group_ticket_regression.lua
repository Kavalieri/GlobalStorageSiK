...
-- The registry mutation above is now represented by a newly certified capture
-- for subsequent independent ticket-context checks.
registry._snapshotRevision.net = 10
...
for sequence = 1, #expectedSizes do
	...
	local remaining, complete = Tickets.commit(batch, batch.itemIds)
	assert(remaining == 34 - #allIds and complete == (#allIds == 34),
		"commit did not advance exactly the confirmed physical identities")
end
