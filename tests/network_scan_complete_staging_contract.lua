...
return function(deps)
	...
	local snapshotStatus = function() return nil, nil end
]] .. handlerSource .. [[
...
local function newHarness(currentRevision)
	...
	local GS = {
		...
		Index = {
			...
			getSnapshotRevision = function() return currentRevision - 1 end,
		...
		ZoneScanJob = {
			getStatus = function() return { state = "STALE_RETRY" } end,
	...
	deps = {
		...
		scheduleSnapshotSync = function(networkId, retryPlayer, revision)
			...
			deps.pendingSnapshotSync[networkId] = { dueMs = 5000 }
		end,
	...
	GS.Server.onNetworkScanProgress = function(networkId, status)
		evidence.progress = { networkId = networkId, status = status }
	end
...
Support.check(suite, "discarded FAILED revision retries without publishing snapshot", function()
	...
	assert(#evidence.overrides == 1
		and evidence.overrides[1].state == "STALE_RETRY"
		and evidence.overrides[1].reason == "snapshot_stale",
		"stale staging did not preserve explicit snapshot_stale retry state")
	assert(#evidence.commands == 1 and evidence.commands[1].command == "scanProgress"
		and evidence.commands[1].payload.state == "STALE_RETRY",
		"stale staging did not emit only scanProgress")
	assert(#evidence.commands == 1 and evidence.commands[1].command == "scanProgress"
		and evidence.commands[1].payload.nextAttemptMs == 5000
		and evidence.commands[1].payload.inventoryRevision == 11,
		"stale staging did not report retry progress: " .. tostring(evidence.commands[1].payload.nextAttemptMs)
			.. "/" .. tostring(evidence.commands[1].payload.inventoryRevision))
	return true
end)

Support.check(suite, "failed zones plus mutation become stale retry", function()
	local handler, evidence, player = newHarness(12)
	local summary = { _terminalState = "FAILED", _stagedDiscarded = true,
		_startRevision = 11, failedZones = 1, zones = 2 }
	handler("network-A", summary, { Kava = "query" })
	assert(#evidence.retries == 1 and evidence.retries[1].revision == 12,
		"mutation with failed zones did not schedule stale retry")
	assert(#evidence.commands == 1 and evidence.commands[1].command == "scanProgress"
		and evidence.snapshots == 0,
		"mutation with failed zones published an actionResult/snapshot")
	assert(#evidence.overrides == 1 and evidence.overrides[1].state == "STALE_RETRY",
		"mutation with failed zones kept FAILED terminal state")
	return true
end)
