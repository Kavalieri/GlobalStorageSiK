...
Support.check(suite, "runtime rail keeps square 56 px tabs inside the 80 px effective track", function()
	...
	contains(tabs, "local RAIL_CROSS_INSET = 12", "product rail lost the approved cross-axis inset")
	contains(tabs, "railCrossInset = RAIL_CROSS_INSET", "Navigation rail does not consume the approved inset")
	contains(tabs, "math.max(profile.window.railWidth, 56 + 2 * RAIL_CROSS_INSET)",
		"product rail does not expose the effective 80 px extent")
...
Support.check(suite, "Options uses one generated standard surface without internal subtabs", function()
	...
	assert(tabNode.type == "container", "Options root is not the declarative content container")
        ...
        assert(not findNode(tabNode, "options-network-block"), "physical terminal retained network selector")
        for _, nodeId in ipairs({ "options-summary-block",
...
Support.check(suite, "Admin selectors and variable staff text reflow inside safe geometry", function()
	...
	local layout = read(CLIENT .. "GS_AdminDashboard_Layout.lua")
	contains(admin, "SupportLayout.reflow", "staff no longer delegates its live layout")
	contains(layout, "UI.Controls.renderWrappedLinePool(", "staff info remains fixed-line")
	contains(admin, "UI.Window.resolveBounds({", "staff bypasses the shared viewport")
	contains(layout, "local cols = math.min(2, count)", "staff actions have no width-aware composition")
	return true
end)
...
Support.check(suite, "base surfaces mount only after navigation has final geometry", function()
	...
	local addToUiAt = assert(api:find("ui:show()", 1, true))
