...
Support.check(suite, "scan job publishes bounded fractional progress", function()
	...
	assert(job:find("progressDone = math%.min%(total, completed %+ fraction%)"),
		"scan progress does not include the current zone fraction")
...
Support.check(suite, "header consumes real progress and a short scan label", function()
	...
	assert(terminal:find('operationLabel("IGUI_GS_ScanRunningShort"', 1, true),
